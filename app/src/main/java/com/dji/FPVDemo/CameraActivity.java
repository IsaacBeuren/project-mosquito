...
public class CameraActivity extends Activity implements SurfaceTextureListener,OnClickListener{
        ...
        Task<List<Barcode>> result = scanner.process(image)
                .addOnSuccessListener(new OnSuccessListener<List<Barcode>>() {
                    ...
                    public void onSuccess(List<Barcode> barcodes) {
                        if (barcodes.size() > 0){
                            if (barcodes.size() >= 2){
                                TV.setText(barcodes.get(0).getRawValue() + " and " + barcodes.get(1).getRawValue());
                                if(isNumeric(barcodes.get(0).getRawValue())){
                                    Mu = Integer.parseInt(barcodes.get(0).getRawValue());
                                    Loc = Integer.parseInt(barcodes.get(1).getRawValue());
                                }else{
                                    Mu = Integer.parseInt(barcodes.get(1).getRawValue());
                                    Loc = Integer.parseInt(barcodes.get(0).getRawValue());
                                }
                            }else{
    ...
    private void QrSend(Integer Location, Integer MaterialUnit) {
            ...
            public void run() {
                try {
                    ...
                    if (conn.getResponseCode() == 200){
                        ...
                        TV.setText("Please read it again!");
                        ToneGenerator toneGen1 = new ToneGenerator(AudioManager.STREAM_MUSIC, 50);
